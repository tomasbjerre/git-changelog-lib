...
public class GitChangelogApi {
 ...
 private Changelog getChangelog(GitRepo gitRepo) throws GitChangelogRepositoryException {
  ObjectId fromId = getId(gitRepo, this.settings.getFromRef(), this.settings.getFromCommit()) //
    .or(gitRepo.getCommit(ZERO_COMMIT));
  Optional<ObjectId> toIdOpt = getId(gitRepo, this.settings.getToRef(), this.settings.getToCommit());
  ObjectId toId;
  ...
  GitRepoData gitRepoData = gitRepo.getGitRepoData(fromId, toId, this.settings.getUntaggedName(),
 ...
 private String getTemplateContent() {
  ...
  try {
   ...
   try {
    ...
    throw new RuntimeException("Cannot find on classpath (" + this.settings.getTemplatePath() + ") or filesystem ("
